...
class TestCron extends Command
{
    ...
    public function createDatabasebackup($host, $database, $user, $password) 
    {
        ...
        try {
            ...
            $env = env('APP_ENV');
            if ($env == 'local') {
                $command1 = sprintf('C:\xampp\mysql\bin\mysqldump --host=%s %s -u %s -p%s> %s', $host, $database, $user, $password, $storageAt.$filename);
            } else {
                $command1 = sprintf('mysqldump --host=%s %s -u %s -p%s > %s', $host, $database, $user, $password, $storageAt.$filename);
            }
            $returnVar = NULL;
            $output = NULL;
            ...
            if ($returnVar == 0) {
                ...
                if (File::exists($backupFilePath)) {
                    ...
                    if ($urlPath) {
                        info("Database backup cron job  completed at ". now());
                    } else {
                        info("Database backup cron job encountered an error at ", now());
                    }
                }
            } else {
            ...
            info("Error backing up database ERR-MSG". $e->getMessage());
        }
    } 
